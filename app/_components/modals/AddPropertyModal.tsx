...
const AddPropertyModal = (props: Props) => {
  ...
  return (
    <Modal label="Add property" isOpen={addPropertyModal.isOpen} onClose={addPropertyModal.close} >
      {currentStep == 1 ? (
        <>
        ...
        <>
          ...
          <div className="pt-3 pb-6 space-y-4">
            <SelectCountry
              value={dataCountry} 
              onChange={(value: SelectCountryValue) => setDataCountry(value)} 
            />
          </div>
        ...
        <>
          ...
          {errors.map((error, index) => {
            return (
              <div key={index} className="p-5 mb-4 bg-roomie text-white rounded-xl opacity-80">
                {error}
              </div>
            )
          })}
